...
import {DesignerContext, NS_Emits, NS_XGraphComLib, T_XGraphComDef} from '@sdk';
...
import ModuleStart from "./module-start/ModuleStart";

require('./ToplComModelForked')
...
export default function ToplCom({model}: { model: ToplComModel }) {
  ...
  useObservable(ComContext,
    next => {
      ...
      next({model, comDef, context, viewContext, emitLogs, emitMessage, emitSnap, emitItem, emitModule, emitPage})
    },
    {to: 'children'})
  ...
  if (comDef.editors && comDef.editors["@render"]) {
    return <Custom/>
  } else {
    ...
    } else if (comDef.namespace === NS_XGraphComLib.coms.module) {
      return <Module/>
    ...
    ({po: {x, y}, epo: {ex, ey}, dpo: {dx, dy}}, state) => {
      if (state === 'start') {
        ...
        if (model.parent instanceof FrameModel) {
          if (model.parent.currentDiagram) {
            parentPo = getPosition(model.parent.currentDiagram.$el)
          } else {
      ...
      if (state == 'finish') {
        if (dragToFrame) {
          //if (model.parent.id !== dragToFrame.id) {
