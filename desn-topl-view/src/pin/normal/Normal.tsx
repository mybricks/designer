...
export default function Normal({click, mousedown, help}) {
  ...
  const exeVal = useComputed(() => {
    ...
    if (exe) {
      ...
      if (model.isDirectionOfInput() && model instanceof PinModelForked) {
        if (from instanceof PinModel) {
          const diagram = model.getRoot(parent => parent instanceof DiagramModel) as DiagramModel
          if (diagram.searchCon(from.id, model.id)) {
            showF = true
          }
        }
      } else {
